...
class Tree {
    constructor(array) {
        ...
        this.root = this.buildTree(filteredAndSortedArr, 0, filteredAndSortedArr.length - 1); 
    }
    ...
    insert(value, node = this.root) {
        ...
        if (value === node.value) {
            throw new Error(`Enter a unique number. This number "${value}" already exists.`);
        }
        if (value < node.value) {
...
function makeRandomNumberArray(min, max, lengthValue = 1){
    return Array.from({ length: lengthValue }, 
